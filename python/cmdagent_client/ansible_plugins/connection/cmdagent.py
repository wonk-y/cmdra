"""Ansible connection plugin backed by cmdagentd."""
...
from cmdagent_client.client import Client
from cmdagent_client.gen.agent.v1 import agent_pb2

DOCUMENTATION = r"""
connection: cmdagent
short_description: Run Ansible modules through cmdagentd
description:
  - Uses the cmdagent Python SDK to execute commands and transfer files over mTLS.
options:
  address:
    description: cmdagentd host:port
    vars:
      - name: ansible_cmdagent_address
  ca_cert:
    description: CA certificate path
    vars:
      - name: ansible_cmdagent_ca_cert
  client_cert:
    description: Client certificate path
    vars:
      - name: ansible_cmdagent_client_cert
  client_key:
    description: Client private key path
    vars:
      - name: ansible_cmdagent_client_key
  server_name:
    description: TLS server name override
    vars:
      - name: ansible_cmdagent_server_name
"""

param(
  ...
  [string]$UIBinary = (Join-Path $PSScriptRoot "..\cmdagentui.exe"),
...
function New-TempDir {
  $path = Join-Path ([System.IO.Path]::GetTempPath()) ("cmdagentui-pty-smoke-" + [guid]::NewGuid().ToString("N"))
...
function Wait-ForPort {
  ...
  throw "cmdagentd did not become reachable on ${Host}:${Port} within ${TimeoutSeconds}s"
}
...
try {
  ...
  Write-Host ""
  Write-Host "cmdagentd is running for interactive cmdagentui PTY validation."
  Write-Host ""
  ...
  $uiExit = $LASTEXITCODE
  if ($uiExit -ne 0) {
    throw "cmdagentui exited with code $uiExit"
  }
}
finally {

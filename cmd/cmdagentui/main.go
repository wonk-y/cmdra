...
import (
	...
	"cmdagent/internal/buildinfo"
	"cmdagent/internal/tui"
...
func run(args []string) int {
	if len(args) > 0 && args[0] == "version" {
		fmt.Fprintln(os.Stdout, buildinfo.Summary("cmdagentui"))
		return 0
	}
	...
	if err := tui.Run(client, cmdagentclient.DialConfig{
		Address:            cfg.address,
		CAFile:             cfg.caFile,
		ClientCertFile:     cfg.clientCertFile,
		ClientKeyFile:      cfg.clientKeyFile,
...
func parseConnectionFlags(args []string) (connectionFlags, error) {
	fs := flag.NewFlagSet("cmdagentui", flag.ContinueOnError)
	fs.SetOutput(new(bytes.Buffer))
	cfg := connectionFlags{address: "127.0.0.1:8443", timeout: 15 * time.Second}
	fs.StringVar(&cfg.address, "address", cfg.address, "cmdagentd address")
